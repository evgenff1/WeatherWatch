...
final class DataUpdater {
    ...
    func startUpdatingCities() {
        ...
        // Запускаем асинхронное обновление городов в задаче с приоритетом background.
        Task.detached(priority: .background) {
            await self.updateCitiesInBackground(cityData: citiesData)
        }
    }
    ...
    private func updateCitiesInBackground(cityData: [(ObjectId, String)]) async {
            for (cityId, cityName) in cityData {
                do {
                    // Выполнение сетевого запроса с использованием async/await.
                    let results = try await NetworkManager.shared.searchCity(query: cityName)
                    // Если нет подходящего результата, переходим к следующему городу.
                    guard let matchedCity = results.first else { continue }
                    
                    // Обновление записи в базе Realm.
                    // Используем autoreleasepool для контроля за потреблением памяти
                    autoreleasepool {
                        do {
                            let realm = try Realm()
                            // Находим город по первичному ключу.
