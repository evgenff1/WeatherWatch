...
final class NetworkManager {
    ...
    /// - Returns: Объект `ForecastResponse` с данными о погоде.
    /// - Throws: Ошибку `AFError`, если запрос не удался.
    func fetchWeatherData(latitude: Double, longitude: Double) async throws -> ForecastResponse {
         ...
         let dataTask = AF.request(url)
             .validate()
             .serializingDecodable(ForecastResponse.self)
    ...
    /// - Parameter query: Название города для поиска.
    /// - Returns: Массив объектов `CitySearchResult`, содержащий найденные города.
    /// - Throws: Ошибку `AFError`, если запрос завершился неудачно.
