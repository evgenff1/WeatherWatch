...
extension CitiesListViewController: UISearchBarDelegate {
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        // Создаем асинхронную задачу, чтобы вызвать async метод
        Task { @MainActor in
            do {
                // Вызываем async функцию, которая может выбросить ошибку
                let results = try await NetworkManager.shared.searchCity(query: searchText)
                // Обновляем UI (на главном потоке)
                self.searchResults = results
                self.citiesListView.tableView.reloadData()
            } catch {
                print("Ошибка при поиске города: \(error)")
                // При ошибке очищаем результаты поиска
                self.searchResults = []
                self.citiesListView.tableView.reloadData()
            }
        }
