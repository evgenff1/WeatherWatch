...
final class NetworkManager {
    ...
    /// Выполняет сетевой запрос по переданному URL и обрабатывает результат.
    /// - Parameters:
    ///   - url: URL запроса.
    ///   - completion: Замыкание, возвращающее `Result` с успешными данными или ошибкой.
    private func performRequest(with url: URL, completion: @escaping (Result<Data, NetworkError>) -> Void) {
        URLSession.shared.dataTask(with: url) { data, response, error in
            // Проверка ошибки сети (например, отсутствие интернет-соединения)
            if let error = error as NSError?, error.domain == NSURLErrorDomain {
                completion(.failure(.noInternetConnection))
                return
            }
            
            // Проверка HTTP-статуса ответа (ожидаемый диапазон 200...299)
            if let httpResponse = response as? HTTPURLResponse,
               !(200...299).contains(httpResponse.statusCode) {
                completion(.failure(NetworkError.error(from: httpResponse.statusCode)))
                return
            }
            ...
            // Возвращаем успешный результат с полученными данными
    ...
    ///     либо ошибку `NetworkError`.
    func fetchWeatherData(latitude: Double, longitude: Double, completion: @escaping (Result<ForecastResponse, NetworkError>) -> Void) {
    ...
    ///     либо ошибку `NetworkError`.
    func searchCity(query: String, completion: @escaping (Result<[CitySearchResult], NetworkError>) -> Void) {
